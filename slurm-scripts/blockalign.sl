...
if [ ! -e sorted/${BLOCK}.done ]; then
	...
	CMD="${BWA} mem -M -t ${SLURM_JOB_CPUS_PER_NODE} -R @RG'\t'$RG_ID'\t'$RG_PL'\t'$RG_PU'\t'$RG_LB'\t'$RG_SM $REFA $READ1 $READ2 | ${SAMTOOLS} view -bh - > $SHM_DIR/align_${BLOCK}.bam"
	...
	CMD="$(which java) ${JAVA_ARGS} -jar ${PICARD} SortSam ${PIC_ARGS} ${SORT_ARGS} INPUT=$SHM_DIR/align_${BLOCK}.bam OUTPUT=$SHM_DIR/sorted_${BLOCK}.bam"
	echo "$HEADER: ${CMD}" | tee -a ../commands.txt
